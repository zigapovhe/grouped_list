...
void main() {
      ...
      home: Scaffold(
        body: GroupedListView(
          ...
          groupSeparatorBuilder: (dynamic element) =>
              Padding(padding: const EdgeInsets.symmetric(vertical: 20), child: Text('$element')),
          interdependentItemBuilder: (_, dynamic previousElement, dynamic currentElement, dynamic nextElement) =>
              Text('${previousElement?['name']} - ${currentElement?['name']} - ${nextElement?['name']}'),
        ),
      ),
    );
  }

  testWidgets('Finds previous, current and next expected elements using interdependency builder',
      (WidgetTester widgetTester) async {
    var testCase1 = 'Tests with empty list';
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements.sublist(0, 0)));
    ...
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements.sublist(0, 1)));
    ...
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements.sublist(0, 2)));
    ...
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements.sublist(0, 3)));
    ...
    expect(find.text('Beth - Will - Miranda'), findsOneWidget, reason: testCase5);
    expect(find.text('Will - Miranda - Mike'), findsOneWidget, reason: testCase5);
    expect(find.text('Miranda - Mike - Danny'), findsOneWidget, reason: testCase5);
    ...
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements, reverse: true));
    ...
    expect(find.text('Miranda - Mike - Danny'), findsOneWidget, reason: testCase6);
    expect(find.text('Will - Miranda - Mike'), findsOneWidget, reason: testCase6);
    expect(find.text('Beth - Will - Miranda'), findsOneWidget, reason: testCase6);
    ...
    var testCase7 = 'Tests with list containing all elements sorted in reverse order';
    await widgetTester.pumpWidget(buildAppWithInterdependency(_elements, sortOrder: GroupedListOrder.DESC));
    ...
    expect(find.text('Danny - Mike - Miranda'), findsOneWidget, reason: testCase7);
    expect(find.text('Mike - Miranda - Will'), findsOneWidget, reason: testCase7);
    expect(find.text('Miranda - Will - Beth'), findsOneWidget, reason: testCase7);
  ...
  testWidgets('finds elemets and group separators with reverse list', (WidgetTester tester) async {
